	...
	void QPBO<REAL>::maxflow(bool reuse_trees, bool _keep_changed_list)
{
	...
	if (!nodeptr_block)
	{
		nodeptr_block = new DBlock<nodeptr>(NODEPTR_BLOCK_SIZE_QPBO, error_function);
	}
	...
	if (keep_changed_list)
	{
		if (!changed_list) changed_list = new Block<Node*>(NODEPTR_BLOCK_SIZE_QPBO, error_function);
	}
