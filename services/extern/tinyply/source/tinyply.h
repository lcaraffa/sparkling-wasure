...
namespace tinyply
{
    ...
    struct PlyFile
    {
         ...
         */ 
      bool parse_header(std::istream & is,char nl_header);
         ...
         */
      void write(std::ostream & os, bool isBinary,char nl_header,bool print_elem);
...
struct PlyFile::PlyFileImpl
{
    ...
    void write_header(std::ostream & os,char nl_header);
    void write_ascii_internal(std::ostream & os,char nl_header, bool print_elem);
    void write_binary_internal(std::ostream & os,char nl_header);
    void write_property_ascii(Type t, std::ostream & os, uint8_t * src, size_t & srcOffset,char nl_char);
...
bool PlyFile::PlyFileImpl::parse_header(std::istream & is,char nl_header)
{
    std::string line;
    while (std::getline(is, line,nl_header))
    {
...
void PlyFile::PlyFileImpl::write_property_ascii(Type t, std::ostream & os, uint8_t * src, size_t & srcOffset,char nl_char)
{
    switch (t)
    {
...
void PlyFile::PlyFileImpl::write(std::ostream & os, bool _isBinary,char nl_header,bool print_elem)
{
	// reset cursors
	for (auto & d : userData) { d.second.cursor->byteOffset = 0; }
	if (_isBinary) write_binary_internal(os,nl_header);
	else write_ascii_internal(os,nl_header,print_elem);
}

void PlyFile::PlyFileImpl::write_binary_internal(std::ostream & os,char nl_header)
{
    isBinary = true;
    write_header(os,nl_header);
...
void PlyFile::PlyFileImpl::write_ascii_internal(std::ostream & os, char nl_header, bool print_elem)
{
  write_header(os,nl_header);
  std::string nl_char = " ";
  if(print_elem){
...
void PlyFile::PlyFileImpl::write_header(std::ostream & os, char nl_header)
{
    const std::locale & fixLoc = std::locale("C");
    os.imbue(fixLoc);

    os << "ply" << nl_header;
    if (isBinary) os << ((isBigEndian) ? "format binary_big_endian 1.0" : "format binary_little_endian 1.0") << nl_header;
    else os << "format ascii 1.0" << nl_header;
        ...
        for (const auto & p : e.properties)
        {
            if (p.isList)
            {
                os << "property list " << PropertyTable[p.listType].str << " "
                   << PropertyTable[p.propertyType].str << " " << p.name << nl_header;
            }
            else
            {
                os << "property " << PropertyTable[p.propertyType].str << " " << p.name << nl_header;
            }
        }
    }
    os << "end_header" << nl_header;
}
...
PlyFile::~PlyFile() { };
bool PlyFile::parse_header(std::istream & is, char nl_header) { return impl->parse_header(is,nl_header); }
void PlyFile::read(std::istream & is) { return impl->read(is); }
void PlyFile::write(std::ostream & os, bool isBinary,char nl_header,bool print_elem) { return impl->write(os, isBinary,nl_header,print_elem); }
